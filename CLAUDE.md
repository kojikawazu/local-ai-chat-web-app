...
- **2層構成**: ユニットテスト（Vitest / 純ロジック）+ E2Eテスト（Playwright / 実環境）。UIコンポーネント単位のUTは不要。
...
- **3分類必須**: UT・E2Eとも正常系・準正常系・異常系に分割。
- **CI**: push/PR to main で自動実行。UT（`unit-test`）とE2E（`e2e-test`）は独立ジョブ。CIモデル: `qwen2.5:0.5b`

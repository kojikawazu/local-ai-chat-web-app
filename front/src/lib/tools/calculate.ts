...
/**
 * 数式を計算して結果を返すエージェントツール。
 *
 * 引数 `expression` に四則演算・括弧・単項マイナスを含む計算式を受け取り、
 * 自前の再帰下降パーサーで安全に評価する（`eval` は使わない）。許可文字以外の
 * 混入・ゼロ除算・不正な形式はエラー文字列として返す。返り値は
 * `"式 = 結果"` 形式の文字列で、実行時に例外は投げず全てを文字列で返す。
 */

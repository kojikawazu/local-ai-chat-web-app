...
- [ ] ツール関連の型定義作成（`lib/tools/types.ts` — OllamaToolDefinition, OllamaToolCall, ToolCallRecord, AgentStreamEvent）
- [ ] ツールレジストリ実装（`lib/tools/index.ts` — registerTool, getTool, getAllToolDefinitions, executeTool）
- [ ] フロントエンド型拡張（`types/index.ts` — Message に metadata フィールド追加、ToolCallInfo 型追加）

#### 21-1-2: 基本ツール実装

- [ ] get_current_datetime ツール実装（`lib/tools/get-current-datetime.ts`）
- [ ] calculate ツール実装（`lib/tools/calculate.ts` — 安全な数式パーサー、許可文字の正規表現制限）

#### 21-1-3: Ollama 連携拡張

- [ ] `lib/ollama.ts` 拡張 — OllamaChatMessage に tool_calls, role: 'tool' 対応追加
- [ ] `lib/ollama.ts` 拡張 — streamChat に tools パラメータ追加
- [ ] OllamaStreamChunk 型拡張 — tool_calls フィールド追加

#### 21-1-4: エージェントループ実装

- [ ] `lib/agent.ts` 作成 — ツール呼び出しループ（最大ラウンド制限付き）
- [ ] Ollama レスポンス解析 — テキスト応答 / tool_calls の判定分岐
- [ ] ツール実行 → tool ロールでの結果返送 → Ollama 再呼び出しの一連フロー
- [ ] 複数ツール同時呼び出し対応（tool_calls 配列の並列実行）

#### 21-1-5: API 変更

- [ ] POST /api/chat 拡張 — enableTools パラメータ追加、NDJSON イベントストリーム対応
- [ ] GET /api/tools 新規作成 — 利用可能ツール一覧取得
- [ ] エラーハンドリング — ツール実行失敗、タイムアウト、最大ラウンド超過

#### 21-1-6: DB スキーマ変更

- [ ] Message モデルに metadata (Json?) フィールド追加
- [ ] Prisma マイグレーション作成・実行
- [ ] メッセージ保存 API — metadata フィールド対応

#### 21-1-7: フロントエンド基盤

- [ ] useChat フック拡張 — NDJSON パーサー追加、プレーンテキストとの後方互換
- [ ] useChat フック拡張 — ツール状態管理（activeToolCall）、メタデータ蓄積
- [ ] ToolCallIndicator コンポーネント実装（ツール実行中のスピナー表示）
- [ ] ToolCallResult コンポーネント実装（実行結果の折りたたみ表示）
- [ ] ChatWindow 変更 — ツール呼び出し情報の表示統合

#### 21-1-8: 設定 UI

- [ ] 設定モーダルにエージェント ON/OFF トグル追加
- [ ] エージェント設定の状態管理（localStorage 永続化）
- [ ] 環境変数追加（AGENT_ENABLED, AGENT_MAX_TOOL_ROUNDS, AGENT_TOOL_TIMEOUT_MS）

#### 21-1-9: E2E テスト（Phase A）

- [ ] エージェント基盤 E2E テスト作成（agent.spec.ts: 正常系/準正常系/異常系）
- [ ] ツール呼び出し表示テスト（インジケーター表示、結果表示）
- [ ] エージェント ON/OFF 切替テスト
- [ ] エラーハンドリングテスト（ツール実行失敗、タイムアウト）

### 21-2: Phase B — 調査系ツール追加

#### 21-2-1: Web 検索ツール

- [ ] web_search ツール実装（`lib/tools/web-search.ts` — DuckDuckGo HTML API）
- [ ] 検索結果のパース・整形（タイトル、URL、スニペット）
- [ ] レスポンスサイズ制限（上位5件、3,000文字以内）

#### 21-2-2: URL 取得ツール

- [ ] url_fetch ツール実装（`lib/tools/url-fetch.ts`）
- [ ] HTML タグ除去・テキスト抽出処理
- [ ] SSRF 防止（プライベート IP アドレスへのアクセスブロック）
- [ ] レスポンスサイズ制限（5,000文字以内）

#### 21-2-3: フロントエンド拡張

- [ ] Web 検索結果のリッチ表示（リンク付き検索結果一覧）
- [ ] URL 取得結果の表示改善

#### 21-2-4: E2E テスト（Phase B）

- [ ] Web 検索ツール E2E テスト作成（正常系/準正常系/異常系）
- [ ] URL 取得ツール E2E テスト作成（正常系/準正常系/異常系）
- [ ] 複数ツール連携テスト（検索 → URL取得の連続実行）

### 21-3: Phase C — 自律エージェント化

#### 21-3-1: システムプロンプト設計

- [ ] 自律思考を促すシステムプロンプト作成（調査→判断→再調査→まとめ）
- [ ] システムプロンプトの設定 UI（設定モーダルでプロンプト選択・カスタマイズ）
- [ ] プリセットプロンプト作成（リサーチャー、分析者、比較レビュアー等）

#### 21-3-2: 思考過程の可視化

- [ ] AgentThinking コンポーネント実装（思考とツール実行の時系列表示）
- [ ] thinking イベントのストリーミング対応（NDJSON に thinking type 追加）
- [ ] 思考過程の折りたたみ UI（デフォルト展開、クリックで折りたたみ）

#### 21-3-3: エージェントループの強化

- [ ] LLM の思考テキスト抽出（ツール呼び出し前の判断テキストを thinking イベントとして送信）
- [ ] 自律判断の品質改善（プロンプトチューニング）
- [ ] 実行統計の表示（ラウンド数、合計ツール呼び出し数、所要時間）

#### 21-3-4: E2E テスト（Phase C）

- [ ] 自律エージェント E2E テスト作成（正常系/準正常系/異常系）
- [ ] 思考過程表示テスト（AgentThinking の表示・折りたたみ）
- [ ] 複数ラウンドの自律調査テスト

### 21-4: ドキュメント

- [ ] `docs/09-agent-architecture.md` 最終更新（実装結果の反映）
- [ ] 既存ドキュメント更新（requirements, architecture, API設計, testing, CLAUDE.md）

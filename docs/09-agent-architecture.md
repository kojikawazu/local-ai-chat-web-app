...
「DNS 解決 → チェック → fetch 実行」のステップ間で DNS が書き換わった場合（DNS rebinding）、チェック通過後にプライベート IP へ接続できる時間差の問題がある。根本的な対策は DNS 解決後の IP アドレスに直接接続することだが、実装コストが高い。**本設計ではローカル個人利用前提のため、`redirect: 'error'` と組み合わせてリスクを限定する方針とし、この制限事項を許容する。**

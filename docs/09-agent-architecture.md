...
| `::ffff:0:0/96` | IPv4-mapped IPv6 | IPv4 プライベートへのマッピング（例: `::ffff:127.0.0.1`） |

**重要**:
- ホスト名（例: `localhost`）を先に DNS 解決し、解決後の IP アドレスに対してブロックリストを適用する。これにより DNS rebinding 攻撃も防止できる。
- Node.js の `net` モジュールで IP をパースする際、IPv4-mapped IPv6 形式（例: `::ffff:192.168.1.1`）で返ってくる場合がある。この場合は IPv4 アドレス部分（`192.168.1.1`）に正規化した上でブロックリストと照合すること。

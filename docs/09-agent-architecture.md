...
| `done` | `metadata?: { toolCalls: ToolCallRecord[] }` | ストリーミング完了 | A |
...
- **`AGENT_TOOL_TIMEOUT_MS = 30000`**: **リトライを含むツール実行全体**のタイムアウト（1試行ごとではない）。`web_search` のリトライ（最大2回 + 待機）を含めた合計時間が30秒を超えた時点でタイムアウトとする。Web 検索や URL 取得はネットワーク遅延を考慮して30秒に設定。ローカルツール（日時取得、計算）は数ミリ秒で完了するため影響しない。

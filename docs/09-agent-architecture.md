...
```json
{"model":"qwen3-coder:latest","message":{"role":"assistant","content":"","tool_calls":[{"function":{"name":"get_current_datetime","arguments":{"timezone":"Asia/Tokyo"}}}]},"done":false}
...
フロントエンドは最初のバイトを確認し、`{` で始まれば NDJSON、そうでなければプレーンテキストとして処理する。
...
| 戻り値 | 計算結果の文字列 |
| 実装 | 安全な数式パーサー（許可する文字: 数字、四則演算、括弧、小数点のみ） |
| セキュリティ | 任意コード実行の禁止。正規表現で許可する文字を限定 |
...
| 実装 | DuckDuckGo HTML API を使用（API キー不要） |
...
| セキュリティ | プライベート IP アドレスへのアクセス禁止（SSRF 防止） |
...
- **url_fetch ツール**: SSRF 防止のため、プライベート IP アドレスへのアクセスを禁止
